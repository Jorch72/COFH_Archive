...
public class DebugTickHandler {
	...
	public void printChunkEvent(ChunkEvent event) {
		...
		DebugHelper.info("[" + event.getChunk().xPosition + "," + event.getChunk().zPosition + "]_" + (event.getChunk().getWorld().isRemote ? "Client" : "Server"));
	}
	...
	public void showParticles(TickEvent.WorldTickEvent evt) {
		...
		if (evt.phase == TickEvent.Phase.START || Minecraft.getMinecraft().theWorld == null || Minecraft.getMinecraft().theWorld.provider.getDimension() != evt.world.provider.getDimension()) {
			return;
		}
